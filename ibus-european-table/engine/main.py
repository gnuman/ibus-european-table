...
try:
    db_dir = os.path.join (os.getenv('IBUS_EUROPEAN_TABLE_LOCATION'),'european-tables')
    icon_dir = os.path.join (os.getenv('IBUS_EUROPEAN_TABLE_LOCATION'),'icons')
except:
    db_dir = "/usr/share/ibus-european-table/european-tables"
    icon_dir = "/usr/share/ibus-european-table/icons"
...
opt.add_option('--no-debug', '-n',
        ...
        help = 'redirect stdout and stderr to ~/.ibus/european-tables/debug.log, default: %default')
...
if (not options.xml) and options.debug:
    if not os.access ( os.path.expanduser('~/.ibus/european-tables'), os.F_OK):
        os.system ('mkdir -p ~/.ibus/european-tables')
    logfile = os.path.expanduser('~/.ibus/european-tables/debug.log')
...
class IMApp:
    def __init__(self, dbfile, exec_by_ibus):
        ...
        if exec_by_ibus:
            self.__bus.request_name("org.freedesktop.IBus.EuropeanTable", 0)
        else:
            self.__component = ibus.Component("org.freedesktop.IBus.EuropeanTable",
                                              "Table Component",
                                              "0.1.0",
                                              "GPL",
